            ...
            <div className="bg-white rounded-lg shadow">
              <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                ...
                <label className="px-6 py-2 bg-blue-600 text-white rounded-lg cursor-pointer hover:bg-blue-700 transition-colors font-medium inline-flex items-center gap-2">
                  ...
                  Upload File
                  <input type="file" onChange={(e) => { setWorkspaceType('private'); onFileChange(e); }} className="hidden" />
                </label>
              </div>
            ...
            <div className="bg-white rounded-lg shadow">
              <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                ...
                <label className="px-6 py-2 bg-blue-600 text-white rounded-lg cursor-pointer hover:bg-blue-700 transition-colors font-medium inline-flex items-center gap-2">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" /></svg>
                  Upload File
                  <input type="file" onChange={(e) => { setWorkspaceType('team'); onFileChange(e); }} className="hidden" />
                </label>
              </div>
                ...
                <form onSubmit={onSearch} className="mb-6">
                  <div className="flex gap-3 items-center">
                    ...
                    <div className="flex items-center gap-3 px-4 py-2 bg-gray-50 rounded-lg border border-gray-300">
                      <span className={`text-sm font-medium ${workspaceType === 'private' ? 'text-blue-600' : 'text-gray-500'}`}>Private</span>
                      <button
                        type="button"
                        onClick={() => setWorkspaceType(workspaceType === 'private' ? 'team' : 'private')}
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${workspaceType === 'team' ? 'bg-green-600' : 'bg-blue-600'}`}
                      >
                        <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${workspaceType === 'team' ? 'translate-x-6' : 'translate-x-1'}`} />
                      </button>
                      <span className={`text-sm font-medium ${workspaceType === 'team' ? 'text-green-600' : 'text-gray-500'}`}>Team</span>
                    </div>
                    <input
                      type="text"
                      value={searchQuery}
